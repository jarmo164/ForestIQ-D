...
def update_owner(o, new_phone, new_address, cur):
    p = prefer_first(o["phone"], new_phone)
    a = prefer_first(o["address"], new_address)
    oid = o["id"]
    print("Updating owner. p=" + str(p) + " a=" + str(a), " oid=" + str(oid))
    cur.execute("update owners set phone = %s, address = %s where id = %s", (p,a,oid,))
...
with open(input_file, newline='', encoding=INPUT_ENCODING) as csvfile:
    ...
    for idx, row in enumerate(csv_reader):
        ...
        col_zero = row[0]
        ik_reg = re.search('(?<=isikukood )(.*)(?=\))', col_zero, re.IGNORECASE)
        if ik_reg:
            owner_id = ik_reg.group()
            new_phone = trim(row[1])
            new_address = trim(row[2])
            ...
            if (owner is None):
                ...
                update_owner(owner, new_phone, new_address, cursor)
