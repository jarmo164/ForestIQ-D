...
const LAYERS: LayerConfig[] = [
  ...
  { key: "subparts", label: "Metsaeraldised", endpoint: "/services/map/layers/subparts", color: "#e58e26", kind: "line", description: "Kõik GeoDjango eraldised" },
  { key: "newSubparts", label: "Uued eraldised", endpoint: "/services/map/layers/new-subparts", color: "#21a366", kind: "fill", description: "Viimase 7 päeva CQL-avastused" },
...
function popupHtml(layer: LayerConfig, properties: MapProperties) {
  const fieldSets: Record<string, [string, string][]> = {
    notifications: [["Teatise nr", "notificationNumber"], ["Katastritunnus", "cadastreId"], ["Eraldis", "subpartCode"], ["Töö liik", "workCode"], ["Staatus", "state"], ["Pindala", "subpartArea"], ["Puuliik", "treeType"], ["Eraldis avastatud", "discoveredAt"]],
    registry: [["Nimetus", "title"], ["Katastritunnus", "cadastreId"], ["Eraldis", "subpartCode"], ["Töö liik", "workCode"], ["Otsus", "decision"], ["Pindala", "area"]],
    default: [["Katastritunnus", "cadastreId"], ["Eraldis", "subpartCode"], ["Puuliik", "treeType"], ["Pindala", "area"], ["Avastatud", "discoveredAt"]],
  };
  const rows = (fieldSets[layer.key] ?? fieldSets.default).filter(([, key]) => properties[key] !== "" && properties[key] != null).map(([label, key]) => `<div style="margin:6px 0"><div style="font-size:10px;letter-spacing:.08em;text-transform:uppercase;color:#627469">${escapeHtml(label)}</div><div style="font-size:13px;font-weight:600;color:#17342a">${escapeHtml(properties[key])}</div></div>`).join("");
  return `<div style="min-width:190px;padding:2px 2px 4px"><div style="font-family:Georgia,serif;font-size:17px;font-weight:700;color:#17342a">${escapeHtml(layer.label)}</div>${rows}</div>`;
}
...
export default function MapWorkspace() {
  ...
  const [visible, setVisible] = useState<Record<LayerKey, boolean>>({ cadastres: true, subparts: true, newSubparts: true, registry: true, notifications: true });
  const [counts, setCounts] = useState<Record<LayerKey, number>>({ cadastres: 0, subparts: 0, newSubparts: 0, registry: 0, notifications: 0 });
  ...
  useEffect(() => {
    ...
    instance.on("load", async () => {
      try {
        ...
        const nextCounts = { cadastres: 0, subparts: 0, newSubparts: 0, registry: 0, notifications: 0 };
        for (const { layer, collection } of collections) {
          ...
          if (layer.kind === "fill") {
            const opacity = layer.key === "newSubparts" ? 0.36 : 0.2;
            instance.addLayer({ id: `${layer.key}-fill`, type: "fill", source: layer.key, paint: { "fill-color": layer.color, "fill-opacity": opacity } });
            instance.addLayer({ id: `${layer.key}-outline`, type: "line", source: layer.key, paint: { "line-color": layer.color, "line-width": layer.key === "newSubparts" ? 2.4 : 1.5 } });
          } else if (layer.kind === "point") instance.addLayer({ id: `${layer.key}-point`, type: "circle", source: layer.key, paint: { "circle-radius": 7, "circle-color": layer.color, "circle-stroke-color": "#ffffff", "circle-stroke-width": 1.8 } });
          else instance.addLayer({ id: `${layer.key}-line`, type: "line", source: layer.key, paint: { "line-color": layer.color, "line-width": 2.2, "line-opacity": 0.88 } });
          ...
          instance.on("click", interactionLayer, event => {
            const properties = event.features?.[0]?.properties as MapProperties | undefined;
            if (!properties) return;
            setSelected({ layer: layer.label, properties });
            popup.current?.remove();
            popup.current = new maplibregl.Popup({ closeButton: true, maxWidth: "290px", offset: 12 }).setLngLat(event.lngLat).setHTML(popupHtml(layer, properties)).addTo(instance);
          });
        }
        setCounts(nextCounts);
        setStatus(`${Object.values(nextCounts).reduce((sum, count) => sum + count, 0)} GeoDjango objekti viiel kaardikihil`);
