...
export default function Contracts() {
  ...
  return <AppShell title="Lepingute tööala" eyebrow="KOMMERTS / LEPINGUD">
    ...
    {tab === "history" && <section className="panel contracts-panel"><div className="table-toolbar contracts-filters"><label htmlFor="contract-search" className="grid gap-1"><span className="text-xs font-semibold">Otsi lepinguid</span><span className="flex items-center gap-2"><Search size={16} /><input id="contract-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Number, müüja või ostja" /></span></label><label htmlFor="contract-status-filter">Olek<select id="contract-status-filter" value={status} onChange={(event) => setStatus(event.target.value)}><option value="ALL">Kõik</option><option value="ACTIVE">Aktiivne</option><option value="ARCHIVED">Arhiveeritud</option></select></label><label htmlFor="contract-from-date">Alates<input id="contract-from-date" type="date" value={fromDate} onChange={(event) => setFromDate(event.target.value)} /></label><label htmlFor="contract-to-date">Kuni<input id="contract-to-date" type="date" value={toDate} onChange={(event) => setToDate(event.target.value)} /></label></div>
