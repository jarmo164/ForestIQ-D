...
export default function P1Operations() {
  ...
  const refreshDeals = useCallback(async () => {
    ...
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Tehingute tervist ei saanud laadida."); }
  ...
  const refreshReferenceData = useCallback(async () => {
    ...
    } catch (reason) { setError(reason instanceof Error ? reason.message : "P1 viiteandmeid ei saanud laadida."); }
  }, []);
  ...
  const setNextAction = async (deal: DealWorkItem) => {
    ...
    try {
      ...
      await refreshDeals();
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Järgmist tegevust ei saanud salvestada."); }
  };

  const markLost = async (deal: DealWorkItem) => {
    const draft = draftFor(deal.id);
    ...
    try {
      ...
      await refreshDeals();
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Tehingut ei saanud sulgeda."); }
  };
  ...
  const qualityAction = async (issue: QualityIssue, operation: "ASSIGN" | "RELEASE" | "RESOLVE") => {
    ...
    try {
      ...
      await refreshQuality();
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Andmekvaliteedi kirjet ei saanud uuendada."); }
  };
  ...
  const addLossReason = async () => {
    ...
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Kaotuse põhjust ei saanud lisada."); }
  };
  ...
  const toggleLossReason = async (reason: LossReason) => {
    try {
      ...
      await refreshReferenceData();
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Kaotuse põhjust ei saanud muuta."); }
  };
  ...
  const openContract = async (contract: ContractRecord) => {
    ...
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Lepingu P1 olekut ei saanud laadida."); }
  };
  ...
  const createReplacementVersion = async () => {
    ...
    } catch (cause) { setError(cause instanceof Error ? cause.message : "Asendusversiooni ei saanud luua."); }
  };
