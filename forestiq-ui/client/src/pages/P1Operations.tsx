...
export default function P1Operations() {
  ...
  return <AppShell title="Operatsioonide kontroll" eyebrow="P1 / MÜÜK JA LEPINGUD">
    ...
    <div className="contracts-tabs" role="tablist">
      <button className={tab === "deals" ? "active" : ""} onClick={() => setTab("deals")}>Tehingu tervis</button>
      <button className={tab === "quality" ? "active" : ""} onClick={() => setTab("quality")}>Andmekvaliteet</button>
      <button className={tab === "losses" ? "active" : ""} onClick={() => setTab("losses")}>Kaotuse põhjused</button>
      <button className={tab === "contracts" ? "active" : ""} onClick={() => setTab("contracts")}>Lepingukontroll</button>
    </div>

    {tab === "deals" && <section className="space-y-4">
