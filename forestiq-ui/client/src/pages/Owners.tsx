...
export default function Owners() {
  ...
  return <AppShell title="Omanikud" eyebrow="FORESTIQ / OMANIKE REGISTER">
    <section className="section-lead"><div><p>Leia kontakt cursor-põhisest registrist, ava katastri seosed ning registreeri järgmine otsus.</p><strong><UsersRound size={17} /> {owners.length} laaditud kirjet</strong></div><div className="search-bar flex-wrap"><label htmlFor="owner-register-search" className="w-full text-xs font-semibold text-muted-foreground">Otsi omanikku</label><Search size={18} /><input id="owner-register-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Nimi, isikukood, telefon või e-post" /><button type="button" aria-label="Filtreeri"><SlidersHorizontal size={18} /></button></div></section>
